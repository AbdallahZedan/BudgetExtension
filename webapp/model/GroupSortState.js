sap.ui.define([
	...
	return BaseObject.extend("ExtensionApproval.ExtensionApproval.model.GroupSortState", {
		 ...
		 * @alias ExtensionApproval.ExtensionApproval.model.GroupSortState
		 */
		...
		/**
		 * Sorts by RequestId, or by Amount
		 *
		...
		/**
		 * Groups by Amount, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Amount") {
				...
				this._oViewModel.setProperty("/sortBy", "Amount");

				aSorters.push(
					new Sorter("Amount", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "RequestId");
			}
