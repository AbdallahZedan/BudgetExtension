		...
		onTheDetailPage : {
			...
			assertions : {
				...
				theLineItemsListShouldHaveTheCorrectNumberOfItems : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "InboxToAttachmentNav",
						success : function (aEntitySet) {
							...
							return this.waitFor({
								...
								check: function (oList) {
									...
									var sObjectID = oList.getBindingContext().getProperty("WorkItem");

									var iLength = aEntitySet.filter(function (oLineItem) {
										return oLineItem.WorkItem === sObjectID;
									}).length;
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							return this.waitFor({
								id : "lineItemsHeader",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name: "text", value: "<InboxToAttachmentNavPlural> (" + iNumberOfItems + ")"}),
								success : function () {
